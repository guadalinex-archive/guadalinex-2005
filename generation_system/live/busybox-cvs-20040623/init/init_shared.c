...
extern void signal_all_processes(int sig)
{
	pid_t sid = -1, pid;
	procps_status_t *p;
	...
	pid = getpid();
        ...
        /* XXX - init crashes and burns if it calls getsid(0) (from a
         * signal handler?) -mdz */
        if (pid != 1)
             sid = getsid(0);
...
#define RB_ENABLE_CAD	0x89abcdef
	...
	/* Stop all processes to avoid process creation races */
	/* TODO: will leave system useless if we run out of memory before
	 * SIGCONTing!
	 */
	//kill(-1, SIGSTOP);
