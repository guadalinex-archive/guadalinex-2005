 ...
 * Modified andenhanced by Anoosh Nader from ADI on May22,2002 to
 * support PPPoA
...
"$Id: ppp.c,v 1.3 2004/08/26 20:47:54 sleeper Exp $";
 ...
 * ARGUMENTS:	
 *  packcket -- buffer in which to place PPPoE packet	
 * RETURNS:
 *  Nothing	
 * DESCRIPTION:	
 *  Reads from a synchronous PPP device and builds and transmits a
 *  complete PPPoA packet	
 */
void
SyncReadFromPPP(PPPoAPacket *packet)
{
    int r;
#ifndef HAVE_N_HDLC
    /* Use scatter-read to throw away the two PPP frame address bytes */
    struct iovec vec[2];
    ...
    unsigned char buf[ETH_DATA_LEN - PPPOA_OVERHEAD];
    r = read(0, buf, ETH_DATA_LEN - PPPOA_OVERHEAD);
    if (r >= 2) {
    ...
    if (r < 0)
    {
        ...
        if (errno == EAGAIN)
        {
            FatalErr("Linux select bug hit!  This message is harmless, but please ask the Linux kernel developers to fix it.");
        }
    ...
    if (r < 2) {
        FatalErr("too few characters read from PPP (SyncReadFromPPP)");
    }
    SendPPPPacket(packet, r);
}
 ...
 * %ARGUMENTS:	
 * packet -- buffer in which to place PPPoA packet	
 * %RETURNS:
 * Nothing
 * %DESCRIPTION:
 * Reads from an async PPP device and builds a complete PPPoA  packet
 * to transmit
 */
void
AsyncReadFromPPP(PPPoAPacket *packet)
{
    ...
    while(r)
    {
        ...
        /* Skip the second PPP frame byte: */
        ...
        while(r && PPPState == STATE_BUILDING_PACKET)
        {
            ...
            switch(c)
            {
                case FRAME_ESC:
                    ...
                    break;
                case FRAME_FLAG:
                    SendPPPPacket(packet, PPPPacketSize);
                    PPPPacketSize = 0;
                    PPPXorValue = 0;
                    PPPState = STATE_WAITFOR_FRAME_ADDR;
                    break;
                default:
                    ...
                    {
                        syslog(LOG_ERR,
                               "Buffer too small (%u); Packet from pppd "
                               "dropped! Lower mtu of pppd, on ppp interface"
                               " or change pppoa source.",
