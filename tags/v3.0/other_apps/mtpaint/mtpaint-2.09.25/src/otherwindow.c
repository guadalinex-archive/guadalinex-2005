...
GtkWidget *pscale_window;
GtkWidget *spin_pscales[8];
int pscale_vals[8];


gint delete_pscale( GtkWidget *widget, GdkEvent *event, gpointer data )
{
	gtk_widget_destroy(pscale_window);

	return FALSE;
}

gint pscale_ok( GtkWidget *widget, GdkEvent *event, gpointer data )
{
	int i;

	for (i=0; i<8; i++)
	{
		gtk_spin_button_update( GTK_SPIN_BUTTON(spin_pscales[i]) );
		pscale_vals[i] = gtk_spin_button_get_value_as_int( GTK_SPIN_BUTTON(spin_pscales[i]) );
	}

	if ( pscale_vals[0] != pscale_vals[4] )		// Only do something if index values are different
	{
		spot_undo();

		mem_scale_pal( pscale_vals[0], pscale_vals[1], pscale_vals[2], pscale_vals[3],
			pscale_vals[4], pscale_vals[5], pscale_vals[6], pscale_vals[7] );

		init_pal();
		update_all_views();
		gtk_widget_queue_draw( drawing_col_prev );
		delete_pscale( NULL, NULL, NULL );
	}
...
{
	GtkWidget *vbox3, *hbox4, *table3;
	GtkWidget *button1, *button2;
	GtkAccelGroup* ag = gtk_accel_group_new();

	int i, j, k;
	char *tab_tex[] = { _("Index"), _("Red"), _("Green"), _("Blue") };

	pscale_window = add_a_window( GTK_WINDOW_TOPLEVEL, _("Create Scaled Palette"),
		GTK_WIN_POS_CENTER, TRUE );

	vbox3 = gtk_vbox_new (FALSE, 0);
	gtk_widget_show (vbox3);
	gtk_container_add (GTK_CONTAINER (pscale_window), vbox3);

	table3 = add_a_table( 3, 5, 10, vbox3 );

///	TABLE TEXT
	for ( i=0; i<4; i++ ) add_to_table( tab_tex[i], table3, 0, i+1, 0, GTK_JUSTIFY_LEFT, 0, 0.5 );
	add_to_table( _("Start"), table3, 1, 0, 0, GTK_JUSTIFY_LEFT, 0, 0.5 );
	add_to_table( _("Finish"), table3, 2, 0, 0, GTK_JUSTIFY_LEFT, 0, 0.5 );
...
///	TABLE SPINBUTTONS
	for ( i=0; i<8; i++ )
	{
		j = 255*(i/4);
		k = 255;
		if ( i == 4 || i == 0 ) k = mem_cols - 1;
		if ( i == 4 ) j = mem_cols - 1;
		spin_to_table( table3, &spin_pscales[i], 1+i/4, 1+i%4, 5, j, 0, k );
	}

	add_hseparator( vbox3, -2, 10 );
	...
	gtk_signal_connect(GTK_OBJECT(button1), "clicked", GTK_SIGNAL_FUNC(delete_pscale), NULL);
	gtk_widget_add_accelerator (button1, "clicked", ag, GDK_Escape, 0, (GtkAccelFlags) 0);

	button2 = add_a_button(_("OK"), 5, hbox4, TRUE);
	gtk_signal_connect(GTK_OBJECT(button2), "clicked", GTK_SIGNAL_FUNC(pscale_ok), NULL);
	...
	gtk_signal_connect_object (GTK_OBJECT (pscale_window), "delete_event",
		GTK_SIGNAL_FUNC (delete_pscale), NULL);

	gtk_widget_show (pscale_window);
	gtk_window_add_accel_group(GTK_WINDOW (pscale_window), ag);
}
