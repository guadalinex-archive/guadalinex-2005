...
int valid_cdrom() {
	...
	/* test "ubuntu" directory */
	if(stat("ubuntu", &dir) != 0)
		return(EXIT_FAILURE);
	if(!S_ISDIR(dir.st_mode))
		return(EXIT_FAILURE);
...
/*	debconf_set(debconf, "cdrom-checker/start", "true");*/
	...
	while(1) {
		...
		debconf_input(debconf, "medium", "cdrom-checker/nextcd");
		debconf_go(debconf);
	...
	debconf_input(debconf, "medium", "cdrom-checker/firstcd");
	debconf_go(debconf);
