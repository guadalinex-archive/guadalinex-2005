		...
		if ( !continuation_line ) {
			...
			if ( col ) {
				...
				*col = 0;
				...
				if ((k_version >= MODUTILS_MINIMAL_VERSION_2_6) && ( *(col-3) == '.' ) &&
						( *(col-2) == 'k' ) && ( *(col-1) == 'o' ))
					ext = 3;
				else
#endif
...
#if defined(CONFIG_FEATURE_2_6_MODULES)
				if ((k_version >= MODUTILS_MINIMAL_VERSION_2_6) && ( *(next-2) == '.' ) && *(next-1) == 'k'  &&
						( *next == 'o' ))
					ext = 3;
				else
#endif
...
#if defined(CONFIG_FEATURE_2_6_MODULES)
	if ((k_version >= MODUTILS_MINIMAL_VERSION_2_6) && ( mod [lm-3] == '.' ) &&
